...
（自動入力をうたう外部ツールは規約違反にあたります）。そのため `/free.html` では
「メルカリに送る」ボタンで **商品説明を自動でコピー → 写真つきの共有メニューを開く**
ところまでを行い、出品画面での貼り付けだけが手作業として残ります。
共有メニューに対応していない端末では、コピー用のボタンと出品画面へのリンクを表示します。
